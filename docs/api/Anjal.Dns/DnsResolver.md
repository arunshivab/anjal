...
- **CreateFromSystem** *(method)* - Construct a resolver pointing at the first IPv4 system DNS server, falling back to 1.1.1.1 if none can be determined. Never throws.
- **FindSystemDnsServer** *(method)* - The first IPv4 DNS server from , else from , else null. A source that fails is skipped rather than allowed to throw.
...
- **ParseResolvConf** *(method)* - The first IPv4 nameserver in resolv.conf text, or null. Comment lines (# or ;) and IPv6 servers are skipped.
