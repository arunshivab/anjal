...
- **IsBodylessWrite** *(method)* - True for a POST or PUT that declares neither a Content-Length header nor chunked transfer encoding - the requests the listener answers with 411 before Anjal sees them (DEF-053). The header itself is tested: measured on .NET 10 on Linux, such a request reports ContentLength64 = 0, exactly like a legitimate empty body, so the number cannot tell them apart.
